...
class VectorStore:
    ...
    def create_tables(self) -> None:
        ...
        create_tables() is a dedicated function from the timescale-vector-library
        and it handles the schema creation on its own
        """
        self.vec_client.create_tables()

    def create_index(self) -> None:
        """
        Create the DiskANN index to speed up similarity search
        create_embedding_index() is a dedicated function from the timescale-vector-library
        and it handles the index creation on its own
        """
        self.vec_client.create_embedding_index(client.DiskAnnIndex())

    def drop_index(self) -> None:
        """
        Drop the DiskANN index in the database
        """
        self.vec_client.drop_embedding_index()

    def upsert(self, df: pd.DataFrame) -> None:
        """
        Batch Upsert (Insert or update) records in the database from a pandas DataFrame.
    ...
    def search(
        ...
        results = self.vec_client.search(query_embeddings, **search_args)
        elapsed_time = time.time() - start_time
